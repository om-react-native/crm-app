...
import { MapPin, Search, Filter as FilterIcon } from 'lucide-react-native';
        ...
        <View style={styles.filtersContainer}>
          ...
          <View style={styles.filterRow}>
            ...
            <CustomDropdown
              value={country}
              onChange={setCountry}
              options={['All', 'USA', 'China', 'Germany', 'UK', 'France', 'Japan', 'India']}
            />
          </View>
        </View>
      )}
        ...
        <ScrollView style={styles.resultsContainer}>
          {searchResults.map((result) => (
            <TouchableOpacity key={result.id} style={styles.resultItem}>
                ...
                <View style={styles.resultMeta}>
                  {result.needsSource && (
                    <View style={[styles.badge, styles.needsSourceBadge]}>
                      <Text style={[styles.badgeText, styles.needsSourceText]}>Needs Source</Text>
                    </View>
                  )}
                  {result.verified && (
                  ...
                  {result.status && (
                    <View style={[
                      styles.badge,
                      result.status === 'Active' ? styles.activeBadge :
                      result.status === 'Inactive' ? styles.inactiveBadge :
                      styles.pendingBadge
                    ]}>
                      <Text style={[
                        styles.badgeText,
                        result.status === 'Active' ? styles.activeText :
                        result.status === 'Inactive' ? styles.inactiveText :
                        styles.pendingText
                      ]}>{result.status}</Text>
                    </View>
                  )}
              ...
              <View style={[
                styles.typeBadge,
                result.type === 'Customer' ? styles.customerBadge : styles.supplierBadge
              ]}>
                <Text style={[
                  styles.typeText,
                  result.type === 'Customer' ? styles.customerText : styles.supplierText
                ]}>{result.type}</Text>
              </View>
            </TouchableOpacity>
          ))}
